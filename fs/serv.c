...
openfile_alloc(struct OpenFile **o)
{
	...
	for (i = 0; i < MAXOPEN; i++) {
		...
		case 0:
			if ((r = sys_page_alloc(0, opentab[i].o_fd, PTE_V|PTE_U|PTE_W|PTE_R)) < 0)
				return r;
			/* fall through */
		case 1:
...
try_open:
	...
	*pg_store = o->o_fd;
	*perm_store = PTE_V|PTE_U|PTE_W|PTE_R|PTE_SHARE;
...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	int r;
	struct OpenFile *o;
	// Step 1. use openfile_lookup to find the relevant open file
	if((r = openfile_lookup(envid, req->req_fileid, &o)) < 0)
		return r;
