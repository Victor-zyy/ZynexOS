...
spawn(const char *prog, const char **argv)
{
	...
	child_tf.sepc = elf->e_entry;
	/* FIXME:  */
	uintptr_t t_sp;
	if ((r = init_stack(child, argv, &t_sp)) < 0)
		return r;
	child_tf.sp = t_sp;
	/* FIXME:  */
...
init_stack(envid_t child, const char **argv, uintptr_t *init_esp)
{
	...
	if ((r = sys_page_alloc(0, (void*) UTEMP, PTE_V|PTE_U|PTE_W|PTE_R)) < 0)
		return r;
	...
	*init_esp = UTEMP2USTACK(&argv_store[-2]);
	...
	if ((r = sys_page_map(0, UTEMP, child, (void*) (USTACKTOP - PGSIZE), PTE_V | PTE_U | PTE_W | PTE_R)) < 0)
		goto error;
	...
	int fd, size_t filesz, off_t fileoffset, int perm)
{
	...
	for (i = 0; i < memsz; i += PGSIZE) {
		if (i >= filesz) {
			...
			// from file
			if ((r = sys_page_alloc(0, UTEMP, PTE_V|PTE_U|PTE_W|PTE_R)) < 0)
				return r;
...
copy_shared_pages(envid_t child)
{
  ...
  int r;
  if((r = sys_copy_shared_pages(child)) < 0){
    return r;
  }
  return 0;
}
