...
malloc(size_t n)
{
	...
	for (i = 0; i < n + 4; i += PGSIZE){
		...
		if (sys_page_alloc(0, mptr + i, PTE_V|PTE_U|PTE_W|PTE_R|cont) < 0){
			for (; i >= 0; i -= PGSIZE)
...
free(void *v)
{
	...
	//sys_uvpt_pte(c) /* FIXME: uvpt[PGNUM(c)]  */
	while (sys_uvpt_pte(c) & PTE_CONTINUED) {
		sys_page_unmap(0, c);
		c += PGSIZE;
