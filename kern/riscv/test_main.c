...
static void CfiFlashWriteBuf(uint32_t wordOffset, const uint32_t *buffer, size_t words, volatile uint32_t *p)
{
	...
	cprintf("p : 0x%08x words : 0x%08x blkAddr : 0x%08x\n", p, words, blkAddr);
...
int CfiFlashWrite(const uint32_t *buffer, uint32_t offset, uint32_t nbytes)
{
    ...
    volatile uint8_t *pbase = (uint8_t *)0x20000000;
...
int CfiFlashErase(uint32_t offset)
{
    ...
    uint8_t *pbase = (uint8_t *)0x20000000;
    if (pbase == NULL) {
        return FLASH_ERROR;
    }
...
void test_main()
{
  ...
  CfiFlashQuery();
  CfiFlashRead((uint32_t *)buf, 0, sizeof(buf));
  cprintf("read buf : \n");
  for(int i = 0 ; i < 4; i++)
    cprintf("buf[%d] : 0x%08x\n", i, buf[i]);
  CfiFlashWrite((uint32_t *)data, 0x400000, sizeof(data));
#if 0
  uint32_t *p = (uint32_t *)0x20000000;
  p[0] = CFIFLASH_CMD_BUFWRITE;
  // read status; 
  volatile uint32_t *base = (uint32_t *)0x20000000;
  int status = base[0];
  cprintf("status : 0x%08x\n", status);
#endif
  CfiFlashRead((uint32_t *)buf, 0, sizeof(buf));
  cprintf("read buf after write : \n");
  for(int i = 0 ; i < 4; i++)
    cprintf("buf[%d] : 0x%08x\n", i, buf[i]);
  while(1);
}
