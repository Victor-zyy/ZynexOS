...
{
	uint32_t addr = pci_conf1_set_addr(f->bus->busno, f->dev, f->func, off);
	return read_w( PCIE_MAP_ECAM_ADDR + addr);
}
...
{
	uint32_t addr = pci_conf1_set_addr(f->bus->busno, f->dev, f->func, off);
	write_w(PCIE_MAP_ECAM_ADDR + addr, v);
}
